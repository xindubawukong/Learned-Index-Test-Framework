...
struct ordered_map {
  ...
  std::optional<bool> try_insert(const K& k, const V& v, const bool upsert=false) {
    ...
    int cidx;
    leaf* l;
    std::tie(p, cidx, l) = verlib::do_now([&] {return find_and_fix(root, k);});
  ...
  std::optional<std::optional<V>> try_find(const K& k) {
    ...
    int cidx;
    leaf* l;
    std::tie(p, cidx, l) = find_no_fix(root, k);
