  ...
  {                                                                            \
    if (!(expr)) {                                                             \
      fprintf(stderr, "Thread %d: RT_ASSERT Error at %s:%d, `%s` not hold!\n", \
              parlay::worker_id(), __FILE__, __LINE__, #expr);                \
      exit(0);                                                                 \
    }                                                                          \
  }
...
#define RT_DEBUG(msg, ...)                                                     \
  if (parlay::worker_id() == 0) {                                             \
    printf(GREEN "T%d: " msg RESET "\n", parlay::worker_id(), __VA_ARGS__);   \
  } else if (parlay::worker_id() == 1) {                                      \
        ...
        void freeForEpoch(uint32_t epoch) {
          ...
          for (Node *node: previousFreeList) {
            if (node->is_two) {
              ...
              tree->pending_two[parlay::worker_id()].push(node);
            } else {
        ...
        : BUILD_LR_REMAIN(BUILD_LR_REMAIN), QUIET(QUIET), memory_budget_(memory_budget) {
      {
        ...
        if (!QUIET) {
          printf("initial memory pool size = %lu\n",
                 pending_two[parlay::worker_id()].size());
        }
      }
      if (USE_FMCD && !QUIET) {
        printf("enable FMCD\n");
...
// return that all specializations of this allocator are interchangeable
    ...
    Node *new_nodes(int n) {
      ...
      al[parlay::worker_id()].field += n * sizeof(Node);
    ...
    Item *new_items(int n) {
      ...
      al[parlay::worker_id()].field += n * sizeof(Item);
      for (int i = 0; i < n; ++i) {
    ...
    Node *build_tree_two(T key1, P value1, T key2, P value2, long double _speed, long _time, int _type) {
      ...
      Node *node = NULL;
      if (pending_two[parlay::worker_id()].empty()) {
        node = new_nodes(1);
        node->is_two = 1;
        node->build_size = 2;
        node->size = 2;
        ...
        node = pending_two[parlay::worker_id()].top();
        pending_two[parlay::worker_id()].pop();
    ...
    void destroy_tree(Node *root) {
      ...
      while (!s.empty()) {
        ...
        if (node->is_two) {
          ...
          for (int i = 0; i < node->num_items; i++) node->items[i].typeVersionLockObsolete.store(0b100);;
          for (int i = 0; i < node->num_items; i++) node->items[i].entry_type = 0;
          pending_two[parlay::worker_id()].push(node);
        } else {
          delete_items(node->items, node->num_items);
          delete_nodes(node, 1);
        }
